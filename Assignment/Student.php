<?php
namespace Assignment2;
use lib\Connector;
use lib\Model;
require_once "../lib/model.php";

class Student extends Model
{
    public $id;
    public $name;
    public $email;
    public $age;
    public $mark;
    public $address;

    protected $table = "students";
    public function __construct($id=null, $name=null, $email =null, $age=null, $mark=null, $address=null)
    {
        $this->id = $id;
        $this->name = $name;
        $this->email =$email;
        $this->age = $age;
        $this->mark = $mark;
        $this->address =$mark;
    }

    public function getStudents(){
        $sql = "SELECT * FROM ".$this->getTable();
        $rs = $this->getConn()->query($sql);
        return $this->toArray($rs);
    }

    public function all(){
        $sql = "SELECT * FROM ".$this->getTable();
        $rs = $this->getConn()->query($sql);
        return $this->toArray($rs);
    }

    public function save()
    {
        $sql_text = "INSERT INTO " . $this->getTable() . " (id,name,email,age,address,mark) VALUES(" . (is_null($this->id) ? 'null' : $this->id) . ",'" . $this->name .
            "','" . $this->email . "','.$this->age.','" . $this->address . "','.$this->mark.') ON DUPLICATE KEY UPDATE name =
             '" . $this->name . "',
             email = '".$this->email."', 
              age = '.$this->age.',
              mark = '.$this->mark.',
            address = '".$this->address."';";
        try{
            $this->getConn()->query($sql_text);
        }catch (\Exception $e){
            die($e->getMessage());
        }

    }

    public function find($id){
        $sql_text = "SELECT * FROM ".$this->getTable()." WHERE id =".$id;
        $ary = toArray($this->getConn()->query($sql_text));
        if(count($ary)>0){
            $data = $ary[0];
            return new Student($data["id"],$data["name"], $age["age"], $data["age"],$data["mark"]);
        }
        return null;
    }

    public function delete(){
        $sql_text = "DELETE FROM ".$this->getTable()." WHERE id =".$this->id;
        $this->getConn()->query($sql_text);
    }
}
